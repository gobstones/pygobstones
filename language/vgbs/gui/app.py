...
class GUI(tkinter.Tk):
    ...
    class GobstonesRunHandler(gbs_run.GobstonesRunHandler):
        def __init__(self, gui):
            self.gui = gui
            self.do_log = None
        def success(self, board_repr, result):
            self.gui.tools.board_format.from_string(board_repr, self.gui.viewer.board1)
            self.gui.gobstones_end_run(result)
        def failure(self, exception):
            self.gui._prepare_for_next_run()
            self.gui.editor.end_run()
            self.gui.editor.show_error(exception)
            if self.gui.viewer:
                if isinstance(exception, common.utils.DynamicException):
                    self.gui.viewer.show_dynamic_error()
                else:
                    self.gui.viewer.show_static_error()
            self.gui._running_end()
        def log(self, msg):
            if not self.do_log: self.do_log = self.gui.editor.make_logger()
            self.do_log(msg)
        def partial(self, board_repr):    
            self.gui.tools.board_format.from_string(board_repr, self.gui.viewer.board1)
            self.gui.viewer.revalidate()
            if self.gui.viewer.visible_board != 1:
                self.gui.viewer.show_board1()
            self.gui.viewer.refresh()
        def read_input(self):
            if not self.gui.input_dialog:
                self.gui.input_dialog = InputDialog(self.gui)
            return self.gui.input_dialog.read_key()

    def _prepare_for_next_run(self):        
        handler = GUI.GobstonesRunHandler(self)
